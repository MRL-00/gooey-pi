...
`)
  ...
  test('shows authenticated built-in Prime MCPs in Capabilities', async () => {
    ...
    await expect(confirmation).toContainText('authenticate again')
    await confirmation.getByRole('button', { name: 'Yes, disable' }).click()
    await expect(notion.getByRole('button', { name: 'Enable Notion' })).toBeVisible()
    await expect(notion).toContainText('Sign in with /mcp login notion')
  })
