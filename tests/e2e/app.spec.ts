...
test.describe('Prime Work desktop smoke', () => {
  ...
  test('shows the bundled GooeyPi pet and exposes Orb and Codex Pets settings', async () => {
    ...
    await dismissDrawer.evaluate((node) => Promise.all(node.getAnimations({ subtree: true }).map((animation) => animation.finished)))
    const dismissTarget = page.locator('.pet-dismiss-drawer__target')
