...
describe('SettingsService.update', () => {
  it('applies every field of a full valid patch', async () => {
    ...
    expect(next).toMatchObject({
      ...
      telemetry: false, askUserEnabled: false, cuaDriverMcpEnabled: true, computerUseEnabled: true,
      disabledProviders: ['openai', 'google'], ompDisabledProviders: ['anthropic'],
  ...
  it('rejects unknown keys, invalid enums, and malformed values without persisting', async () => {
    ...
    await expect(service.update({ cuaDriverMcpEnabled: 'yes' })).rejects.toThrow(/must be a boolean/)
    await expect(service.update({ computerUseEnabled: 'yes' })).rejects.toThrow(/must be a boolean/)
