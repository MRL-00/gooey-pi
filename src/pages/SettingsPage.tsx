...
export function SettingsPage({ settings, meta, providerCatalog, onUpdate, onResetBrowser, onOpenDocs, onRefreshProviders, onSaveProviderApiKey, onLogoutProvider, onSetProviderEnabled, onStartProviderOAuth }: SettingsPageProps) {
  ...
  const content = (() => {
    switch (section) {
      ...
      case 'providers': return <ProvidersSettings catalog={providerCatalog} onRefresh={onRefreshProviders} onSaveApiKey={onSaveProviderApiKey} onLogout={onLogoutProvider} onSetEnabled={onSetProviderEnabled} onStartOAuth={onStartProviderOAuth} onOpenDocs={onOpenDocs} />
