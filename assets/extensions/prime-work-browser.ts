/**
 * Prime Work in-app browser control.
 *
 * Loaded by Prime Agent via --extension when the desktop app spawns a
 * runtime. Talks only to Prime Work's loopback capability broker; the URL and
...
async function call(method: string, params: Record<string, unknown>): Promise<Record<string, unknown>> {
  if (!BRIDGE_URL || !BRIDGE_TOKEN) throw new Error('Prime Work browser control is not available in this runtime')
  ...
  try {
    ...
    throw new Error(`Prime Work is not reachable: ${String(error)}`)
  }
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    description: 'Navigate the Prime Work in-app browser tab to a URL, or go back/forward/reload. Returns the resulting page URL and title once loading settles.',
  ...
  pi.registerTool({
    ...
    description: 'Read the Prime Work in-app browser tab as structured data. Mode "interactive" (default) lists clickable/typeable elements with ref numbers usable in browser_click and browser_type; mode "text" also returns the visible page text.',
    promptGuidelines: [
  ...
  pi.registerTool({
    name: 'browser_type',
    label: 'Browser type',
    description: 'Type text into the Prime Work in-app browser tab. Optionally focus an element ref first (from browser_read_page) and press Enter afterwards with submit=true.',
  ...
  pi.registerTool({
    ...
    description: 'Press a keyboard key in the Prime Work in-app browser tab (enter, tab, escape, backspace, delete, arrow keys, home, end, pageup, pagedown, space, or a single character), optionally with shift/control/alt/meta modifiers.',
    parameters: Type.Object({
      key: Type.String(),
  ...
  pi.registerTool({
    ...
    description: 'Scroll the Prime Work in-app browser tab up/down/left/right by an amount in pixels (default 600). Pass x/y coordinates to scroll a nested scrollable region under that point.',
    parameters: Type.Object({
  ...
  pi.registerTool({
    ...
    description: 'Run JavaScript in the Prime Work in-app browser tab and return its JSON-serialized result. The code runs as an async function body, so use return to produce a value.',
