...
- **D3. Dependency hosting** — RESOLVED (2026-08-07) by **vendoring**: the bucket turned out to be third-party (not user-controlled), so the custom-domain plan was dropped. The four release tarballs (~10 MB, verified byte-for-byte against the previously trusted lockfile sha512 hashes before adoption) are committed under `vendor/` and installed via `file:` specifiers + npm `overrides` for the `@earendil-works/pi-*` transitives. Installs no longer touch the publisher's bucket at all. Guards: `scripts/release/dependency-pins.json` + tests assert lockfile↔pin agreement and that the on-disk tarballs hash to the pinned sha512. Upgrades are a deliberate process documented in docs/security.md. 4.38's weekly audit workflow covers CVE monitoring.
