...
- `assets/extensions/` ships both browser-capability extensions: `prime-work-browser.ts` (Prime ExtensionAPI) and `omp-work-browser.ts` (OMP hook API). Both are self-contained by design and speak the same loopback-broker env contract (`PRIME_WORK_BROWSER_*`, deliberately shared).
- Prime-only surfaces: schedules/heartbeats, plugins/skills catalog, provider auth (OMP credentials live in `~/.omp/agent/agent.db`, owned by the omp CLI), and the daemon-socket follow-up. These are hidden in the renderer when OMP is active and rejected at the IPC layer for `harness: 'omp'`.
...
- Persist desktop state via `JsonStateStore.update` (state version 3: harness-scoped projects, `activeHarness`, `ompApprovalMode`); do not directly rewrite Prime or OMP settings or session files. Read `docs/security.md` before changing IPC, paths, processes, browser, Git, plugins, or terminal code, and `docs/omp-integration.md` before changing harness adapters, OMP readers, or the switcher.
