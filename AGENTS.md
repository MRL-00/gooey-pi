...
Prime Work is a macOS Electron desktop client for three agent harnesses: Prime Agent, OMP (oh-my-pi), and base Pi (pi.dev). All three share the same Pi lineage and speak stdio JSONL RPC; the active harness is toggled from the sidebar brand switcher or Settings → Agent, and each harness has its own projects, sessions, model catalog, and runtimes. Built with strict TypeScript, React 19, electron-vite, and `node-pty`. It expects Node 22.12.0+, npm 10.9.0+, and a configured `prime-agent`, `omp`, and/or `pi` executable on `PATH`. The harness integration designs are documented in `docs/omp-integration.md` and `docs/pi-integration.md`.
